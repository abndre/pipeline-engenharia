...
ENV SPARKVERSION=3.5.6

# Copiar o Spark para dentro da imagem
#COPY spark-3.5.5-bin-hadoop3 /opt/spark
RUN curl -fsSL https://dlcdn.apache.org/spark/spark-${SPARKVERSION}/spark-${SPARKVERSION}-bin-hadoop3.tgz \
    | tar -xz -C /opt/ \
    && mv /opt/spark-${SPARKVERSION}-bin-hadoop3 /opt/spark
